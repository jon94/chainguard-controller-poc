...
func (r *ImagePolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for _, deployment := range deployments {
		status := r.analyzeDeploymentCompliance(deployment, imagePolicy.Spec.Repository, latestDigest, enforceLatest)
...
func (r *ImagePolicyReconciler) analyzeDeploymentCompliance(deployment appsv1.Deployment, repository, latestDigest string, enforceLatest bool) securityv1.DeploymentStatus {
	now := metav1.Now()
	...
	for _, container := range deployment.Spec.Template.Spec.Containers {
		if strings.HasPrefix(container.Image, repository) || strings.HasPrefix(container.Image, "docker.io/"+repository) {
			...
			// Check if current digest matches latest
			if enforceLatest && latestDigest != "" && status.CurrentDigest != latestDigest {
				status.IsCompliant = false
			}
			break
		}
